  ...
  return (
    <>
      <section className="relative overflow-hidden">
        <div className="mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-28">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            <p className="mb-5 text-xs font-bold uppercase tracking-[0.3em] text-[#d6b15e]">Gaborone, Botswana · Writer · Speaker · Builder</p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="rounded-lg border border-[#d6b15e]/25 bg-[#111111] p-6 shadow-2xl shadow-black/40">
            <div className="grid aspect-[4/5] place-items-center rounded-lg border border-white/10 bg-[linear-gradient(145deg,#070707,#202020)] p-8 text-center">
              <div>
                <div className="mx-auto grid h-24 w-24 place-items-center rounded-md bg-[#d6b15e] text-4xl font-black text-[#070707]">KG</div>
                <h2 className="mt-8 text-3xl font-black">Build with clarity. Lead with impact.</h2>
                <p className="mt-4 text-sm leading-6 text-[#cfc8b5]">Premium writing, resources, seminars, and advisory experiences for ambitious professionals and entrepreneurs.</p>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="bg-[#0d0d0d] px-4 py-16 sm:px-6 lg:px-8">
        <SectionHeader eyebrow="Free Downloads" title="Resources You Can Use Today" text="Demo PDF resources priced at BWP 0.00 for the MVP preview." />
        ...
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {testimonials.map((item) => (
              <blockquote key={item.name} className="rounded-lg border border-white/10 bg-white/[0.035] p-6">
                <p className="text-sm leading-6 text-[#cfc8b5]">"{item.quote}"</p>
                <footer className="mt-5 text-sm font-bold text-[#d6b15e]">{item.name}<span className="block text-xs font-semibold text-[#8f8879]">{item.role}</span></footer>
              </blockquote>
            ))}
          </div>
        </div>
